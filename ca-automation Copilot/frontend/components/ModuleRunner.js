...
export default function ModuleRunner({ module, endpoint }) {
  ...
  const connectProgress = (progressId) => {
    if (eventSourceRef.current) {
      eventSourceRef.current.close();
    }

    const eventSource = new EventSource(`${API_URL}/progress/${progressId}`);
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const resultData = processRes.data;
  ...
  return (
    <div className="space-y-3">
          ...
          {error.includes('SMTP') && (
            <div className="bg-red-100 rounded p-3 text-xs space-y-1">
              ...
              <ol className="list-decimal list-inside space-y-1 text-red-700">
                <li>Enable 2-Step Verification on <strong>garg1969sanjeev@gmail.com</strong></li>
      ...
      {/* Progress indicator for email sending */}
      {progress && (
        <div className={`border p-4 rounded-lg ${progress.status === 'completed' ? 'bg-green-50 border-green-200' : 'bg-blue-50 border-blue-200'}`}>
          <div className="flex items-center justify-between mb-2">
            <h4 className={`font-medium text-sm ${progress.status === 'completed' ? 'text-green-900' : 'text-blue-900'}`}>
              {progress.status === 'completed' ? '✅ Emails Sent Successfully' : '📧 Sending Emails...'}
            </h4>
            <span className={`text-sm font-semibold ${progress.status === 'completed' ? 'text-green-700' : 'text-blue-700'}`}>
              {progress.percentage}%
            </span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-2 mb-2">
            <div
              className={`h-2 rounded-full transition-all duration-300 ease-out ${progress.status === 'completed' ? 'bg-green-600' : 'bg-blue-600'}`}
              style={{ width: `${progress.percentage}%` }}
            ></div>
          </div>
          <p className={`text-xs ${progress.status === 'completed' ? 'text-green-700' : 'text-blue-700'}`}>
            {progress.message}
          </p>
          {progress.recipient && (
            <p className={`text-xs mt-2 ${progress.status === 'completed' ? 'text-green-700' : 'text-blue-700'}`}>
              {progress.status === 'completed'
                ? 'Last email sent to:'
                : 'Currently sending to:'}{' '}
              <a
                href={`mailto:${progress.recipient}`}
                className="font-mono text-current underline hover:text-blue-800"
                target="_blank"
        ...
        <div className="space-y-3">
          ...
          <div className="bg-green-50 border border-green-200 p-3 rounded-lg">
            ...
            {(result.emailReport?.length > 0 || result.count > 0) && (
              <p className="text-xs text-green-700 mt-1">
                Sent from <span className="font-mono">garg1969sanjeev@gmail.com</span>
              </p>
            )}
          </div>
                    ...
                    {result.clients.map((c, i) => (
                      <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-blue-50'}>
                        ...
                        <td className="px-3 py-2 text-center">
                          <span className={`px-2 py-0.5 rounded text-xs font-semibold ${c.regime === 'New Regime'
                    ...
                    {r.sent
                      ...
                      : (
                        <div className="text-right">
                          ...
                          {r.error && (
                            <p className="text-red-400 text-xs mt-0.5 max-w-[220px] truncate" title={r.error}>
                              {r.error}
                            </p>
                          )}
                        </div>
          ...
          {/* Download links */}
          {(result.download || result.zipUrl || result.invalidEmailReportUrl) && (
            <div className="space-y-2">
              ...
              {result.invalidEmailReportUrl && (
                <a href={result.invalidEmailReportUrl} download className="block btn-secondary text-center text-sm">
                  📋 Download Invalid Email Report
                </a>
              )}
            </div>
          )}
